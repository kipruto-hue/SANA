...
console.log(`frozen: ${protocols.size} protocols, ${steps} steps, ${Object.keys(system.lines).length} system lines`);
console.log(`  library_sha256: ${manifest.library_sha256}`);
console.log(`  audio recorded: ${withAudio}/${steps} steps`);
if (pending.length > 0) {
  console.log(
    `  awaiting clinician sign-off: ${pending.map((p) => p.id).join(', ')} ` +
